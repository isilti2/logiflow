  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 max-w-md text-center space-y-4">
        ...
        <h1 className="text-xl font-black text-gray-900">Link Geçersiz</h1>
        <p className="text-gray-500 text-sm">Bu paylaşım linki geçersiz veya süresi dolmuş.</p>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-3 py-1 rounded-full">Salt Okunur</span>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-start justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-black text-gray-900 text-lg">LogiFlow Yük Planı</h1>
                <p className="text-xs text-gray-400 mt-0.5">{data.containerLabel} · {data.date}</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-sm font-bold text-gray-900">Kargo Listesi</h2>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {data.items.map((item, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-gray-500 font-mono text-xs">{item.dims}</td>
                    <td className="px-5 py-3.5">
                      <span className="inline-flex items-center justify-center w-7 h-7 bg-blue-50 text-blue-700 font-bold text-xs rounded-lg">
                        {item.qty}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-gray-500 text-xs">{item.weight}</td>
                  </tr>
                ))}
              </tbody>
            </table>
