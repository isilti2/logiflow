...
const CAT_STYLE: Record<string, { badge: string; dot: string }> = {
  Teknoloji:            { badge: 'bg-blue-50 text-blue-600 border border-blue-100',      dot: 'bg-blue-500' },
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 space-y-16">
              ...
              <button key={cat}
                className={`text-xs font-semibold px-4 py-2 rounded-full transition-all ${
                  cat === 'Tümü'
                    ...
                    : style ? style.badge : 'bg-gray-100 text-gray-600 border border-gray-200'
                }`}>
                {cat}
              </button>
            );
        ...
        <Link href={`/blog/${featured.id}`}
          className="group block bg-gray-50 border border-gray-100 rounded-3xl overflow-hidden hover:shadow-xl hover:shadow-gray-200/60 hover:-translate-y-0.5 transition-all duration-300">
          <div className="grid md:grid-cols-2">
            ...
            <div className={`h-56 md:h-auto min-h-[220px] bg-gradient-to-br ${CAT_GRADIENT[featured.category] ?? 'from-blue-50 to-white'} flex items-center justify-center relative overflow-hidden`}>
              ...
              <div className="relative text-center">
                <div className="w-20 h-20 bg-white rounded-3xl shadow-lg flex items-center justify-center mx-auto mb-3">
                  <span className="text-2xl font-black text-gray-400">{CAT_ICON[featured.category]}</span>
                </div>
            ...
            <div className="p-8 md:p-10 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-100">Öne Çıkan</span>
                <span className="text-xs text-gray-400">{featured.date} · {featured.readTime} okuma</span>
              </div>
        ...
        {/* Yazı grid */}
        <div>
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Tüm Yazılar</h3>
            ...
            {rest.map((post) => {
              ...
              return (
                <Link key={post.id} href={`/blog/${post.id}`}
                  className="group bg-white border border-gray-100 rounded-3xl overflow-hidden hover:shadow-lg hover:shadow-gray-200/50 hover:-translate-y-0.5 transition-all duration-300 flex flex-col">
                  {/* Görsel alanı */}
                  <div className={`h-36 bg-gradient-to-br ${grad} flex items-center justify-center relative overflow-hidden`}>
                    ...
                    <div className="relative w-12 h-12 bg-white rounded-2xl shadow flex items-center justify-center">
                      <span className="text-lg font-black text-gray-400">{CAT_ICON[post.category]}</span>
                    </div>
                  </div>
                  {/* İçerik */}
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <h2 className="text-base font-black text-gray-900 mb-2 leading-snug group-hover:text-blue-600 transition-colors flex-1">
                      {post.title}
                    </h2>
                    <p className="text-xs text-gray-500 leading-relaxed mb-4 line-clamp-2">{post.summary}</p>
                    <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                      <span className="text-xs text-gray-400">{post.date}</span>
                      <span className="text-xs font-semibold text-blue-600 group-hover:gap-1.5 flex items-center gap-1 transition-all">
                        Oku
