...
export default function FaturaPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="bg-white border-b border-gray-100 dark:border-gray-800 h-16" />
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1,2,3].map(i => <div key={i} className="h-64 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 animate-pulse" />)}
        </div>
      </main>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link href="/profil" className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-blue-600 px-3 py-1.5 rounded-lg transition-colors">
              ← Profilim
            </Link>
            <button
              onClick={async () => { await logout(); window.location.href = '/'; }}
              className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-red-500 border border-gray-200 dark:border-gray-700 px-3 py-2 rounded-xl transition-colors"
            >
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900 dark:text-white">Fatura & Abonelik</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">Planınızı yönetin ve kullanım durumunuzu takip edin.</p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-gray-900 dark:text-white">Mevcut Plan: <span className="text-blue-600">Free</span></p>
            <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">Daha fazla özellik için Pro plana geçin.</p>
          </div>
          <button
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
          <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-5">Bu Ayki Kullanım</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {USAGE_STATS.map(({ icon: Icon, label, used, total }) => (
              <div key={label} className="flex flex-col gap-2">
                ...
                <p className="text-2xl font-black text-gray-900 dark:text-white">
                  {used}
                  {total > 0 && <span className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 font-medium"> / {total}</span>}
                </p>
                {total > 0 && (
        ...
        {/* Plan comparison */}
        <div>
          <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Planlar</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {PLANS.map((plan) => {
              ...
              return (
                <div
                  ...
                  className={`relative bg-white rounded-2xl border shadow-sm p-6 flex flex-col ${
                    isPopular ? 'border-blue-300 ring-2 ring-blue-100' : 'border-gray-100 dark:border-gray-800'
                  }`}
                >
                  {isPopular && (
                  ...
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">{plan.label}</p>
                    <div className="flex items-end gap-1 mt-1">
                      <span className="text-3xl font-black text-gray-900 dark:text-white">{plan.price}</span>
                      {plan.period && <span className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1">{plan.period}</span>}
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">{plan.desc}</p>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                      isCurrent
                        ? 'bg-gray-100 text-gray-500 dark:text-gray-400 dark:text-gray-500 cursor-default'
                        : isPopular
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
            <h2 className="text-sm font-bold text-gray-900 dark:text-white">Fatura Geçmişi</h2>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-left">
                {['Tarih', 'Plan', 'Tutar', 'Durum'].map((h) => (
                  <th key={h} className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {INVOICES.map((inv, i) => (
                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  <td className="px-6 py-3 text-gray-700 dark:text-gray-200 font-medium">{inv.date}</td>
                  <td className="px-6 py-3">
                    <span className="text-xs font-semibold bg-gray-100 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{inv.plan}</span>
                  </td>
                  <td className="px-6 py-3 text-gray-700 dark:text-gray-200 font-mono">{inv.amount}</td>
                  <td className="px-6 py-3">
          ...
          <div className="px-6 py-3 text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800">{INVOICES.length} kayıt</div>
        </div>
