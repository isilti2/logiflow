...
const STATUS_COLOR: Record<string, string> = {
  depolarda: 'bg-blue-50 text-blue-700',
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 text-sm text-gray-400 hover:text-blue-600 border border-gray-200 hover:border-blue-300 rounded-xl px-3 py-1.5 bg-white transition-colors"
        title="Kargo Ara (Ctrl+K)"
      >
        <Search className="w-4 h-4" />
        <span className="hidden lg:inline text-xs">Kargo ara…</span>
        <kbd className="hidden lg:inline text-[10px] bg-gray-100 px-1.5 py-0.5 rounded font-mono text-gray-400">⌃K</kbd>
      </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[10vh] px-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100">
              ...
              <button onClick={() => setOpen(false)} className="text-xs text-gray-400 border border-gray-200 px-1.5 py-0.5 rounded font-mono">esc</button>
            </div>
                  ...
                  {results.map(({ area, items }) => (
                    <div key={area.id}>
                      <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 border-b border-gray-100">
                        ...
                        <span className="ml-auto text-[10px] text-gray-400">{items.length} kalem</span>
                      </div>
                      {items.map((item) => (
                        <button
                          key={item.id}
                        ...
                        >
                          ...
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${STATUS_COLOR[item.status] ?? 'bg-gray-100 text-gray-500'}`}>
                            {item.status}
                          </span>
                        </button>
                      ))}
